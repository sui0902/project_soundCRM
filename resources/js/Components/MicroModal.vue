...
<template>
            ...
            <table class="table-auto w-full text-left whitespace-no-wrap">
                ...
                <tbody>
                    <tr v-for="customer in customers.value.data" :key="customer.id">
                        <td @click="setCustomer({ id: customer.id, kana: customer.kana })" class="border-t-2 border-gray-200 px-4 py-3 text-blue-400">
                          {{ customer.id }}
                        </td>
                        ...
                        <td class="border-t-2 border-gray-200 px-4 py-3 text-lg text-gray-900">{{ customer.tel }}</td>
                    </tr>
                </tbody>
            </table>
          </div>
  ...
  <div class="flex gap-2">
    <input name="customer" v-model="search" class="flex-1 bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-cyan-500 focus:bg-white focus:ring-2 focus:ring-cyan-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
    <button type="button" @click="searchCustomers" data-micromodal-trigger="modal-1" class="text-white bg-cyan-500 border-0 py-2 px-8 focus:outline-none hover:bg-cyan-600 rounded text-lg">検索</button>
  </div>
